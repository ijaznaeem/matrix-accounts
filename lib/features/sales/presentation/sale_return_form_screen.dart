...
class SaleReturnFormScreen extends ConsumerStatefulWidget {
  ...
  const SaleReturnFormScreen({super.key, this.returnId});
...
class _SaleReturnFormScreenState extends ConsumerState<SaleReturnFormScreen> {
  ...
  void _generateReturnNumber() {
    ...
    _returnNoController.text =
        'SR${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}${now.millisecond.toString().padLeft(3, '0')}';
  }
  ...
  Future<List<Party>> _loadCustomers() async {
    ...
    return await isar
        ...
        .and()
        .partyTypeEqualTo(PartyType.customer)
        .findAll();
  }

  Future<List<Invoice>> _loadSalesInvoices() async {
    ...
    return await isar
        ...
        .and()
        .partyIdEqualTo(_selectedCustomer!.id)
        .and()
        .not()
  ...
  Future<void> _saveReturn() async {
    ...
    if (_selectedOriginalInvoice == null) {
      _showError('Please select original sales invoice');
      return;
    }
    ...
    try {
          ...
          if (item.returnQuantity > 0) {
            final stockLedger = StockLedger()
              ...
              ..movementType = StockMovementType
                  .inAdjustment // Returning items increases stock
              ..quantityDelta =
                  item.returnQuantity // Positive because items are coming back
      ...
      if (mounted) {
        _showError('Error creating sale return: $e');
      }
    } finally {
      setState(() {
        _isLoading = false;
