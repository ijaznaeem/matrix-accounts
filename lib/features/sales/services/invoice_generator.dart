...
class InvoiceGenerator {
  ...
  static Future<Uint8List> generateInvoiceImage({
    ...
    try {
      ...
      _drawRightAlignedText(
          canvas,
          '$currencySymbol ${_currencyFormat.format(subTotal)}',
          Offset(750, yPos + 8),
          const TextStyle(
              fontSize: 13,
              fontWeight: FontWeight.bold,
      ...
      _drawText(
          canvas,
          '$currencySymbol ${_currencyFormat.format(invoice.previousBalance)}',
          Offset(600, yPos),
          const TextStyle(fontSize: 14, color: Colors.black87));
      yPos += 25;
      ...
      _drawText(
          canvas,
          '$currencySymbol ${_currencyFormat.format(calculatedTotal)}',
          Offset(600, yPos),
          const TextStyle(
              fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black));
      yPos += 25;
...
💰 Invoice Amount: $currencySymbol ${_currencyFormat.format(invoice.grandTotal)}
💵 Paid: $currencySymbol ${_currencyFormat.format(totalPaid)}
📊 Opening Balance: $currencySymbol ${_currencyFormat.format(invoice.previousBalance)}
📋 Closing Balance: $currencySymbol ${_currencyFormat.format(closingBalance)}
